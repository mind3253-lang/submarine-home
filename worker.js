export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/reservations/cancel" && request.method === "POST") {
      const member=await sessionMember();if(!member)return Response.json({ok:false,error:"로그인이 필요합니다."},{status:401});
      try{const d=await request.json(),rows=await readReservations(),i=rows.findIndex(x=>x.id===d.id&&x.memberNo===member.memberNo);if(i<0)return Response.json({ok:false,error:"예약을 찾을 수 없습니다."},{status:404});const booking=rows[i],now=new Date(),useDate=new Date(booking.date+"T00:00:00"),today=new Date(now.getFullYear(),now.getMonth(),now.getDate()),days=Math.ceil((useDate-today)/86400000);let refundRate=days>=4?100:days===3?70:days===2?50:days===1?30:0;rows.splice(i,1);await writeReservations(rows);const waivers=await readWaivers(),kept=waivers.filter(w=>!(w.waiverCode==="D-1"&&w.memberNo===member.memberNo&&w.lessonDate===booking.date&&String(w.lessonTime).replace("타임","")===String(booking.time).replace("타임","")));if(kept.length!==waivers.length)await writeWaivers(kept);return Response.json({ok:true,refundRate,deletedFacilityWaivers:waivers.length-kept.length})}catch(e){return Response.json({ok:false,error:e?.message||String(e)},{status:500})}
    }
